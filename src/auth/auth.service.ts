...
import {
  UserCreateOptions,
  UserStatusEnum,
} from 'src/user/interfaces/user.interfaces';
...
import {
  TikTokTokenResponse,
  TikTokUserInfo,
} from './interface/tik-tok.interface';
import { SignupUserDto } from './dto/signup-user.dto';
import { DataSource, Repository } from 'typeorm';
import { User } from 'src/user/entity/user.entity';
import { generateOtp } from '@global/utils/globals';
    ...
    try {
      return await this.dataSource.manager.transaction(
        async (entityManager) => {
          ...
          user.verified = true;
          // const { firstName, lastName, phoneNumber, phoneCode } = user;

          if (!user?.wallet) {
            // const paystack_id: string = await this.paystackService.createCustomer({
            //   email: user.email,
            ...
            // const accountDetails = await this.paystackService.createCustomerNuban(
            //   paystack_id,
            //   phone,
            //   firstName,
            //   lastName,
            // );
            // const { currency, accountName, accountNumber, customer, bank, bankCode } =
            //   accountDetails;
            ...
            const wallet = walletRepo.create({
              ...
              // paystackId: customer,
              user,
              // nubanProvider: NubanProvider.PAYSTACK,
              // ...(bankCode && { bankCode: `${bankCode}` }),
            });

            wallet.user = user;

            await walletRepo.save(wallet); // save explicitly
          ...
          delete (rest as any).userRefId;
          delete (rest as any).wallet;
          // await this.emailQueue.add(
          //   JobType.SEND_EMAIL_ZEPTO,
          //   {
          //     recipient: email,
